...
func UserAuthorization(c echo.Context) error {
	...
	config.DB.Where("login = ?", requestUser.Login).First(&user)
	config.DB.Model(user).Related(&user.Role)

	err := bcrypt.CompareHashAndPassword([]byte(user.Pass), []byte(requestUser.Pass))

	if err != nil {
		log.Print(err)
		return c.String(http.StatusForbidden, " false")
	}
